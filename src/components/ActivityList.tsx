...
export default function ActivityList({activities, dispatch}: ActivityListProps) {
  ...
  return (
    <>
      ...
      {isEmptyActivities? <p className="text-center py-5">No hay actividades</p> : activities.map(activity =>(
        <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between shadow-md">
          ...
          <div className="flex gap-5 items-center">
            <button>
              <PencilSquareIcon className="h-8 w-8 text-gray-800 hover:text-gray-400"
              onClick={()=> dispatch({type:'set-activeID', payload:{id:activity.id}})}/>
            </button>
            <button>
              <XCircleIcon className="h-8 w-8 text-red-500 hover:text-red-200"
